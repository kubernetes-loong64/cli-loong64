...
推送 `loong64-cli-v<版本>` 格式的分支（如 `loong64-cli-v29.7.0`）即可触发构建。可追加 `+<build>`
（如 `loong64-cli-v29.7.0+0`）携带构建元数据。

## [发布](https://github.com/kubernetes-loong64/cli-loong64/releases)

推送 `release-loong64-cli-v<版本>` 格式的标签（如 `release-loong64-cli-v29.7.0+0`）即可自动创建 GitHub
Release 并上传构建产物。
